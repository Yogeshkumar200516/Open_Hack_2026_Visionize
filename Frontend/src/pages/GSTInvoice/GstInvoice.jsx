...
import InvoicePreviewModal from "./InvoiceModal.jsx";
...
const menuProps = {
  ...
  const handleBarcodeScan = async (e) => {
    if (e.key === "Enter" && barcode.trim() !== "") {
      try {
        const { data: scannedProduct } = await api.current.get(
          `/api/products/by-barcode/${barcode.trim()}`
        );
        ...
        const rate = parseFloat(scannedProduct.price || 0);
  ...
  const totalProductBaseAmount = products.reduce(
    (acc, item) => acc + parseFloat(item.amount || 0),
    0
  );

  const totalProductGstAmount = products.reduce(
    (acc, item) => acc + (parseFloat(item.amount || 0) * (parseFloat(item.gst || 0) / 100)),
    0
  );

  const subtotal = totalProductBaseAmount; // Base subtotal after item-level discounts
  ...
  // Global GST applies to the discounted subtotal (Other Charges/Processes)
  const otherGstCost = ((subtotal - discountValue) * gst) / 100;

  const gstCost = totalProductGstAmount + otherGstCost;
  ...
  const total = subtotal - discountValue + transportAmount + gstCost;
  ...
  const handleAddSelectedProducts = (selectedProductsObj) => {
    setProducts((prevProducts) => {
      ...
      const selectedMap = new Map(selectedArray.map((p) => [p.product_id, p]));

      // Keep selected products and sync modal-edited values (quantity/rate/etc.)
      const retained = filtered
        ...
        .map((existing) => {
          const selected = selectedMap.get(existing.product_id);
          if (!selected) return existing;

          const quantity = parseFloat(
            selected.quantity ?? existing.quantity ?? 0
          ) || 0;
          const rate =
            parseFloat(
              selected.price ?? existing.rate ?? 0
            ) || 0;
          const gst = parseFloat(selected.gst ?? existing.gst ?? 0) || 0;
          const discount =
            parseFloat(selected.discount ?? existing.discount ?? 0) || 0;

          const baseAmount = quantity * rate;
          const discountAmount = (baseAmount * discount) / 100;
          const netAmount = baseAmount - discountAmount;
          const priceWithGst = netAmount + (netAmount * gst) / 100;

          return {
            ...existing,
            particular: selected.product_name ?? existing.particular,
            hsn_code: selected.hsn_code ?? existing.hsn_code,
            quantity,
            rate,
            gst,
            discount,
            ...
            priceIncludingGst: priceWithGst.toFixed(2),
            stock_quantity: selected.stock_quantity ?? existing.stock_quantity,
          };
        });
      ...
      const newlyFormatted = selectedArray
        ...
        .map((p) => {
          ...
          const rate = parseFloat(p.price ?? 0) || 0;
          const gst = parseFloat(p.gst ?? 0) || 0;
          ...
          return {
            ...
            particular: p.product_name ?? p.particular ?? "",
            hsn_code: p.hsn_code,
            quantity,
            unit: "Kg",
            rate,
  ...
  useEffect(() => {
    async function fetchProducts() {
      try {
        const res = await api.current.get(`/api/products`);
  ...
  const handleProductChange = (index, field, value) => {
    ...
    if (field === "particular") {
      ...
      const baseAmount = quantity * (parseFloat(rate || 0) || 0);
      const discountAmount = (baseAmount * discount) / 100;
  ...
  const handleModalQuantityChange = (productId, newQuantity) => {
      ...
      prevProducts.map((product) =>
        product.product_id === productId
          ? {
              ...
              amount: (newQuantity * product.rate * (1 - (parseFloat(product.discount || 0) / 100))).toFixed(2),
              priceIncludingGst: (newQuantity * product.rate * (1 - (parseFloat(product.discount || 0) / 100)) * (1 + (parseFloat(product.gst || 0) / 100))).toFixed(2),
            }
          : product
      )
    );
    ...
    setSelectedProducts((prevSelected) => ({
      ...
      [productId]: {
        ...
        quantity: newQuantity,
        amount: (newQuantity * prevSelected[productId].rate * (1 - (parseFloat(prevSelected[productId].discount || 0) / 100))).toFixed(2),
        priceIncludingGst: (newQuantity * prevSelected[productId].rate * (1 - (parseFloat(prevSelected[productId].discount || 0) / 100)) * (1 + (parseFloat(prevSelected[productId].gst || 0) / 100))).toFixed(2),
      },
    }));
  };
  ...
  const resetForm = (oldInvoiceNo = "") => {
  const currentDate = getCurrentDate();

  // Reset customer details (pass oldInvoiceNo if available to ensure we increment post-save)
  setCustomer({
    name: "",
    address: "",
    state: "",
  ...
  // Fetch with forceIncrement = true since we just saved
  fetchNextInvoiceNumber(currentDate, oldInvoiceNo, true);
...
const buildInvoicePrefixFromDate = (dateInput) => {
  // Read tenant_id from localStorage and pad to minimum 2 digits (same as backend)
  const tenantId = JSON.parse(localStorage.getItem("user"))?.tenant_id ?? "";
  const tenantIdStr = tenantId !== "" ? String(tenantId).padStart(2, "0") : "";
  ...
  return `INV${tenantIdStr}-${dd}${mm}${yyyy}`;
};

const escapeRegex = (value = "") => value.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");


const getFallbackInvoiceNumber = (dateInput, currentInvoiceNo = "", shouldIncrement = false) => {
  const prefix = buildInvoicePrefixFromDate(dateInput);
  if (!prefix) return currentInvoiceNo || "";

  // Extract the numeric sequence from the end of the existing invoice number if present
  // e.g., "INV03-03042026-002" -> "002"
  const match = String(currentInvoiceNo || "").match(/-(\d+)$/);
  
  // If we have a sequence, decide whether to increment it (+1) or preserve it (+0)
  const baseSeq = match ? parseInt(match[1], 10) : 1;
  const nextSeq = shouldIncrement ? baseSeq + 1 : baseSeq;
  
  // Pad the sequence correctly (001, 002... 1000)
  const seqStr = nextSeq < 1000 ? String(nextSeq).padStart(3, "0") : String(nextSeq);
  
  return `${prefix}-${seqStr}`;
};

const getNextInvoiceFromExistingInvoices = async (dateInput, currentInvoiceNo = "", shouldIncrement = false) => {
  const prefix = buildInvoicePrefixFromDate(dateInput);
  if (!prefix) return getFallbackInvoiceNumber(dateInput, currentInvoiceNo, shouldIncrement);
  const tenantId = JSON.parse(localStorage.getItem("user"))?.tenant_id ?? "";
  const tenantIdStr = tenantId !== "" ? String(tenantId).padStart(2, "0") : "";

  try {
    const res = await api.current.get("/api/invoices/get-invoice");
    const invoices = Array.isArray(res?.data) ? res.data : [];
    const pattern = new RegExp(`^INV${tenantIdStr}-[0-9]{8}-(\\d+)$`);

    let maxSeq = 0;
    invoices.forEach((inv) => {
      const invoiceNo = String(inv?.invoice_number || inv?.invoiceNo || "");
    ...
    // If we found a max sequence in DB, the next is max+1
    const nextSeq = maxSeq + 1;
    const seqStr = nextSeq < 1000 ? String(nextSeq).padStart(3, "0") : String(nextSeq);
    return `${prefix}-${seqStr}`;
  } catch (e) {
    return getFallbackInvoiceNumber(dateInput, currentInvoiceNo, shouldIncrement);
  }
};

const fetchNextInvoiceNumber = useCallback(async (invoiceDate, currentInvoiceNo = "", forceIncrement = false) => {
  if (localStorage.getItem("subscriptionType") === "bill") return "";

  try {
    // ✅ Fix: Use specified date, OR existing select date, OR default to today
    const dateParam = invoiceDate || customer.date || getCurrentDate();
    const res = await api.current.get("/api/invoices/next-number", {
      params: { date: dateParam },
    });

    const nextInvoiceNo = res?.data?.invoice_number;
    if (nextInvoiceNo) {
      setCustomer((prev) => ({ ...prev, invoiceNo: nextInvoiceNo }));
    ...
    // ✅ Fix: Use specified date, OR existing select date, OR default to today
    const dateParam = invoiceDate || customer.date || getCurrentDate();
    const resolvedOldNo = currentInvoiceNo || customer.invoiceNo || "";
    const nextInvoiceNo = await getNextInvoiceFromExistingInvoices(dateParam, resolvedOldNo, forceIncrement);
...
}, [customer.date, customer.invoiceNo]);

useEffect(() => {
  // ✅ Only fetch on INITIAL MOUNT to prevent "Today" resets later
  fetchNextInvoiceNumber();
}, []);

const handleDuplicateSelect = (invoice) => {
  if (!invoice) return;

  // 1. Populate customer state (Buyer + Consignee)
  setCustomer({
    name: invoice.customer?.customer_name || invoice.customer_name || "",
    address: invoice.customer?.customer_address || invoice.customer_address || "",
    state: invoice.customer?.customer_state || invoice.customer_state || "",
    pincode: invoice.customer?.customer_pincode || invoice.customer_pincode || "",
    email: invoice.customer?.customer_email || invoice.customer_email || "",
    whatsapp_number: invoice.customer?.customer_whatsapp_number || invoice.customer_whatsapp_number || "",
    mobile: invoice.customer?.customer_mobile || invoice.customer_mobile || "",
    gst: invoice.customer?.customer_gst_number || invoice.customer_gst_number || "",
    placeOfSupply: invoice.customer?.customer_place_of_supply || invoice.customer_place_of_supply || "",
    vehicleNo: invoice.customer?.customer_vehicle_number || invoice.customer_vehicle_number || "",
    invoiceNo: "", // Fetch new invoice number for today's date
    date: new Date().toISOString().split("T")[0],

    consignee_name: invoice.customer?.consignee_name || invoice.consignee_name || "",
    consignee_address: invoice.customer?.consignee_address || invoice.consignee_address || "",
    consignee_state: invoice.customer?.consignee_state || invoice.consignee_state || "",
    consignee_pincode: invoice.customer?.consignee_pincode || invoice.consignee_pincode || "",
    consignee_email: invoice.customer?.consignee_email || invoice.consignee_email || "",
    consignee_mobile: invoice.customer?.consignee_mobile || invoice.consignee_mobile || "",
    consignee_gst: invoice.customer?.consignee_gst_number || invoice.consignee_gst_number || "",
    consignee_placeOfSupply: invoice.customer?.consignee_place_of_supply || invoice.consignee_place_of_supply || "",
    consignee_vehicleNo: invoice.customer?.consignee_vehicle_number || invoice.consignee_vehicle_number || "",
  });

  // Fetch new invoice number for today's date
  fetchNextInvoiceNumber(new Date().toISOString().split("T")[0]);

  // 2. Populate products state
  if (invoice.items && Array.isArray(invoice.items)) {
    const mappedProducts = invoice.items.map(item => ({
      product_id: item.product_id,
      particular: item.product_name || "",
      hsn_code: item.hsn_code || "",
      quantity: item.quantity || "",
      unit: item.unit || "Kg",
      rate: item.rate || "",
      amount: parseFloat(item.base_amount || 0).toFixed(2),
      gst: item.gst_percentage || "",
      discount: item.discount || "",
      priceIncludingGst: parseFloat(item.total_with_gst || 0).toFixed(2),
      stock_quantity: item.stock_quantity ?? (item.product?.stock_quantity ?? 9999),
    }));
    setProducts(mappedProducts);
  }

  // 3. Populate pricing states
  setDiscount(invoice.discount_value !== null ? String(invoice.discount_value) : "");
  setDiscountType(invoice.discount_type || "%");
  setTransportCharge(invoice.transport_charge !== null ? String(invoice.transport_charge) : "");
  setTransportChecked(parseFloat(invoice.transport_charge || 0) > 0);

  // 4. Eway bill fields
  setEwayBillNo(invoice.eway_bill_no || "");
  if (invoice.eway_bill_date) {
    setEwayBillDate(invoice.eway_bill_date.split("T")[0]);
  }
  setTransporterName(invoice.transporter_name || "");
  setTransporterGstNo(invoice.transporter_gst_number || "");
  setTransportMode(invoice.transport_mode || "Road");
  setTransportDistance(invoice.transport_distance || "");
  setTransactionType(invoice.transaction_type || "Regular");
  setSupplyType(invoice.supply_type || "Outward");
  setPlaceOfDispatch(invoice.place_of_dispatch || "");
  setDocumentType(invoice.document_type || "Tax Invoice");
  if (invoice.eway_valid_upto) {
    setEwayValidUpto(invoice.eway_valid_upto.split("T")[0]);
  }

  // 5. Payment details
  setPaymentType(invoice.payment_type || "Cash");
  setPaymentStatus(invoice.payment_status || "Full Payment");
  setAdvanceAmount(invoice.advance_amount !== null ? String(invoice.advance_amount) : "");
  setDueDate(invoice.due_date || "");

  // Show success snackbar
  setSnackbarMessage("Invoice details duplicated successfully! You can now edit and generate.");
...
const handleCustomerChange = (e) => {
  ...
  setCustomer((prev) => {
    ...
    // If we changed the DATE, update the invoice number prefix LOCALLY
    // this prevents resetting to 001 if the database fetch happens too early.
    if (name === "date" && updated.invoiceNo) {
      const newPrefix = buildInvoicePrefixFromDate(value);
      // Regex matches: {Prefix}-{Sequence}
      const match = String(updated.invoiceNo).match(/-(\d+)$/);
      if (match) {
        const seq = match[1];
        updated.invoiceNo = `${newPrefix}-${seq}`;
      }
    }
  ...
  // Only fetch a brand new number if it's currently missing
  if (name === "date" && !customer.invoiceNo) {
    fetchNextInvoiceNumber(value);
  }
};
...
const handleSubmit = async () => {
  ...
  const normalizedDate = String(customer?.date || "").trim();
  let resolvedInvoiceNo = String(customer?.invoiceNo || "").trim();

  if (subscriptionType !== "bill" && !resolvedInvoiceNo) {
    resolvedInvoiceNo = await fetchNextInvoiceNumber(normalizedDate || getCurrentDate());
    if (resolvedInvoiceNo) {
      setCustomer((prev) => ({ ...prev, invoiceNo: resolvedInvoiceNo }));
    }
  }

  // Validate required fields
  if (subscriptionType === "bill") {
    if (!customer || !normalizedName || !normalizedMobile) {
    ...
    if (!customer || !normalizedName || !normalizedMobile || !normalizedDate || !resolvedInvoiceNo) {
  ...
  const updatedSummary = {
    totalWithGst: parseFloat(safeSubtotal.toFixed(2)),
  ...
  const payload = {
    customer: {
      ...
      date: normalizedDate || customer.date,
      invoiceNo: resolvedInvoiceNo,
      billNo: customer.bill_no || customer.billNo,
    },
    products,
  ...
  const relativeApiUrl = subscriptionType === "bill" ? "/api/bills/create" : "/api/invoices/create";

    setIsLoading(true);
    try {
      const response = await api.current.post(relativeApiUrl, payload);
      ...
      if (response.status === 201 || response.status === 200) {
        ...
        setSnackbarSeverity("success");
        setSnackbarOpen(true);
        // ✅ Pass the SAVED invoice number to resetForm so we can increment to the next one
        resetForm(resolvedInvoiceNo);
      } else {
                ...
                >
                  ...
                  <Grid container spacing={1} sx={{ display: "grid" }}>
                    ...
                    {[
                      ...
                      { label: "Price/Nos", key: "rate" },
                      { label: "GST %", key: "gst" },
                      { label: "Discount %", key: "discount" },
                      { label: "Amount", key: "amount" },
                      { label: "Total (Incl. GST)", key: "priceIncludingGst" },
                    ].map(({ label, key }, i) => {
                      return (
                        ...
                        InputProps={{
                          endAdornment: (
                            ...
                            >
                              ...
                              <MenuItem value="Ton">Ton (MT)</MenuItem>
                            </Select>
                          ),
                        }}
                        sx={{
...
{subscriptionType === "bill" && (
  ...
  // ✅ FIXED TOTAL
  {
    ...
    value: `₹${(subtotal + gstCost).toFixed(2)}`,
  },
]
                      .filter(Boolean)
                      .map((item, idx) => (
  ...
  <Box component="ul" sx={{ listStyle: "none", pl: 0, m: 0 }}>
    {[
      {
        label: gst > 0 ? `GST (+ ${gst}% on Bill)` : "GST",
        value: `₹${(gstCost || 0).toFixed(2)}`,
      },
      {
        label: gst > 0 ? `CGST (+ ${(gst / 2).toFixed(2)}% on Bill)` : "CGST",
        value: `₹${(cgstCost || 0).toFixed(2)}`,
      },
      {
        label: gst > 0 ? `SGST (+ ${(gst / 2).toFixed(2)}% on Bill)` : "SGST",
        value: `₹${(sgstCost || 0).toFixed(2)}`,
      },
      {
        label: "Total GST Value",
        value: `₹${((cgstCost || 0) + (sgstCost || 0)).toFixed(2)}`,
      },
      {
        label: "Grand Total",
              ...
              <Grid key={key} item xs={12} sm={6} md={4}>
                ...
                >
                  ...
                  <Typography
                    ...
                    sx={{
                      fontSize: "1.2rem",
                      visibility: (key !== "invoiceNo" && key !== "vehicleNo") ? "visible" : "hidden"
                    }}
                  >
                    *
                  </Typography>
                ...
                <TextField
                  fullWidth
                  required={key !== "invoiceNo" && key !== "vehicleNo"}
                  name={key}
                  ...
                  InputProps={key === "invoiceNo" ? { readOnly: true } : {}}
                  sx={{
                    ...fieldStyle,
                    ...(key === "invoiceNo" && {
                      "& .MuiInputBase-root": {
                      ...
                      "& .Mui-disabled": {
                        WebkitTextFillColor: isDark ? "#b2b3b4" : "gray",
                      },
                      "& .MuiOutlinedInput-notchedOutline": {
                        borderColor: isDark ? "#555" : "#ccc",
                      },
              ...
              <Grid key={key} item xs={12} sm={6} md={4} sx={{ px: { xs: 0, sm: 1 } }}>
                ...
                >
                  ...
                  <Typography
                    ...
                    sx={{
                      fontSize: "1.2rem",
                      visibility: (key !== "consignee_vehicleNo") ? "visible" : "hidden"
                    }}
                  >
                    *
                  </Typography>
                ...
                <TextField
                  fullWidth
                  required={key !== "consignee_vehicleNo"}
                  name={key}
                  type="text"
                  value={customer[key]}
                  onChange={handleCustomerChange}
...
/>
      ...
      <DuplicateInvoiceModal
        ...
        onSelectInvoice={handleDuplicateSelect}
        themeMode={theme.palette.mode}
      />
    </>
  );
