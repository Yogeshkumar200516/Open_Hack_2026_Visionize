...
import FullInvoiceEditModal from "./FullInvoiceEditModal.jsx";
...
function PartyMaster() {
  ...
  const [advanceBill, setAdvanceBill] = useState(null);
  ...
  const currentUserRole = (JSON.parse(localStorage.getItem("user"))?.role || "").toLowerCase();
    ...
    return items.filter((item) => {
      ...
      if (term) {
        ...
        const matchesSearch =
          ...
          (item.customer_gst_number?.toLowerCase().includes(term) ?? false) ||
...
  return false;
  ...
  const openApproveDialog = (item) => { setActionInvoice(item); setApproveDialogOpen(true); };
  const openRejectDialog  = (item) => { setActionInvoice(item); setRejectionNote(""); setRejectDialogOpen(true); };
  const openDeleteDialog  = (item) => { setActionInvoice(item); setDeleteDialogOpen(true); };

  const handleApprove = async () => {
    if (!actionInvoice) return;
    setActionLoading(true);
    try {
      await axiosInstance.put(`/api/invoices/approve/${actionInvoice.invoice_id}`);
      setSnackbarSeverity("success");
      setSnackbarMessage(`Invoice #${actionInvoice.invoice_number} approved successfully.`);
      setSnackbarOpen(true);
      setApproveDialogOpen(false);
      fetchInvoices();
    } catch (err) {
      setSnackbarSeverity("error");
      setSnackbarMessage(err.response?.data?.message || "Failed to approve invoice.");
      setSnackbarOpen(true);
    } finally {
  ...
  const handleReject = async () => {
    ...
    setActionLoading(true);
    try {
      await axiosInstance.put(`/api/invoices/reject/${actionInvoice.invoice_id}`, {
        rejection_note: rejectionNote,
      });
      setSnackbarSeverity("warning");
      setSnackbarMessage(`Invoice #${actionInvoice.invoice_number} rejected.`);
      setSnackbarOpen(true);
      setRejectDialogOpen(false);
      fetchInvoices();
    } catch (err) {
      setSnackbarSeverity("error");
      setSnackbarMessage(err.response?.data?.message || "Failed to reject invoice.");
      setSnackbarOpen(true);
    } finally {
      setActionLoading(false);
    }
  ...
  const handleDelete = async () => {
    ...
    setActionLoading(true);
    try {
      await axiosInstance.delete(`/api/invoices/delete/${actionInvoice.invoice_id}`);
      setSnackbarSeverity("success");
      setSnackbarMessage(`Invoice #${actionInvoice.invoice_number} deleted and stock restored.`);
      setSnackbarOpen(true);
      setDeleteDialogOpen(false);
      fetchInvoices();
    } catch (err) {
      setSnackbarSeverity("error");
      setSnackbarMessage(err.response?.data?.message || "Failed to delete invoice.");
      setSnackbarOpen(true);
    } finally {
      setActionLoading(false);
    }
  ...
  // Helper: badge colour for invoice_status
  const getStatusChipProps = (status) => {
    if (status === "approved")        return { label: "Approved",  color: "success" };
    if (status === "rejected")        return { label: "Rejected",  color: "error"   };
    if (status === "pending_approval") return { label: "Pending",  color: "warning" };
    return { label: "Legacy", color: "default" };
  };
            ...
            >
              <TableHead>
                <TableRow sx={{ backgroundColor: isDark ? "#121212" : "#f4f4f4", height: "60px" }}>
                  <TableCell sx={{ color: primaryColor, fontWeight: "bold" }}>S.No</TableCell>
                  ...
                  <TableCell sx={{ fontWeight: "bold", color: primaryColor, width: '250px' }}>Actions</TableCell>
                  {subscriptionType !== 'bill' && (
                    <TableCell sx={{ fontWeight: "bold", color: primaryColor }}>Approval Status</TableCell>
                  )}
                </TableRow>
              </TableHead>
              <TableBody>
                {paginatedItems.length === 0 ? (
                  <TableRow>
                    <TableCell colSpan={13} align="center" sx={{ color: palette.text.secondary }}>
                  ...
                  paginatedItems.map((item, index) => (
                    ...
                    >
                      ...
                      <TableCell>
                        ...
                        {/* ── Advance Payment Edit (always visible for Advance invoices) ── */}
                        ...
                        {/* ── Approval Workflow buttons (invoices only) ── */}
                        {subscriptionType !== "bill" && (() => {
                          const status = item.invoice_status;
                          const isAdmin = currentUserRole === "admin" || currentUserRole === "super_admin";
                          const isPending = status === "pending_approval" || !status;
                          return (
                            <>
                              {/* Approve — admin only, visible while pending */}
                              {isPending && isAdmin && (
                                <Tooltip title="Approve Invoice">
                                  <IconButton
                                    ...
                                    sx={{
                                      ...
                                      color: "#43a047",
                                      "&:hover": { boxShadow: "0 0 8px #43a047" },
                                    }}
                                  >
                              ...
                              {/* Reject — admin only, visible while pending */}
                              {isPending && isAdmin && (
                                <Tooltip title="Reject Invoice">
                                  <IconButton
                                    ...
                                    sx={{
                                      ...
                                      color: "#e65100",
                                      "&:hover": { boxShadow: "0 0 8px #e65100" },
                                    }}
                                  >
                              ...
                              {/* Edit — visible while pending AND no sales return */}
                              {isPending && !item.has_sales_return && (
                                <Tooltip title="Edit Invoice (Pending)">
                                  <IconButton
                                    onClick={() => {
                                      setFullEditInvoiceId(item.invoice_id);
                                    ...
                                    sx={{
                                      ...
                                      color: "#1565c0",
                                      "&:hover": { boxShadow: "0 0 8px #1565c0" },
                                    }}
                                  >
                              ...
                              {isPending && item.has_sales_return && (
                                <Tooltip title="Cannot Edit: Invoice has an associated Sales Return">
                                  <span>
                                    <IconButton
  disabled
  sx={(theme) => ({
    ...
    "&.Mui-disabled": {
      ...
      color: theme.palette.mode === "light" ? "#757575" : "#bdbdbd",
      opacity: 1, // Prevent MUI from making it too faded
    },
  })}
>
  <EditNoteIcon fontSize="small" />
                              ...
                              {/* Delete — visible while pending AND no sales return */}
                              {isPending && !item.has_sales_return && (
                                <Tooltip title="Delete Invoice">
                                  <IconButton
                                    ...
                                    sx={{
                                      ...
                                      color: "#e53935",
                                      "&:hover": { boxShadow: "0 0 8px #e53935" },
                                    }}
                                  >
                      ...
                      {subscriptionType !== "bill" && (() => {
                        const { label, color } = getStatusChipProps(item.invoice_status);
                        return (
                          <TableCell>
                            <Chip
                              label={label}
                            ...
                            />
                            {item.invoice_status === "rejected" && item.rejection_note && (
                              <Tooltip title={`Reason: ${item.rejection_note}`}>
                                <Typography
                                  variant="caption"
                                  sx={{ display: "block", color: "error.main", maxWidth: 120, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}
                                >
                                  {item.rejection_note}
                                </Typography>
                              </Tooltip>
      ...
      <Dialog open={approveDialogOpen} onClose={() => setApproveDialogOpen(false)}>
        <DialogTitle sx={{ color: "#43a047", fontWeight: "bold" }}>Approve Invoice</DialogTitle>
        <DialogContent>
          <DialogContentText>
            Are you sure you want to <strong>approve</strong> Invoice{" "}
            <strong>#{actionInvoice?.invoice_number}</strong>?<br />
            Once approved, the invoice will become <strong>read-only</strong> and cannot be edited or deleted.
          </DialogContentText>
        </DialogContent>
        <DialogActions>
      ...
      <Dialog open={rejectDialogOpen} onClose={() => setRejectDialogOpen(false)} fullWidth maxWidth="sm">
        <DialogTitle sx={{ color: "#e65100", fontWeight: "bold" }}>Reject Invoice</DialogTitle>
        <DialogContent>
          <DialogContentText sx={{ mb: 2 }}>
            Reject Invoice <strong>#{actionInvoice?.invoice_number}</strong>?
            The invoice will be marked as <strong>Rejected</strong> and kept for audit purposes.
          </DialogContentText>
          <TextField
      ...
      <Dialog open={deleteDialogOpen} onClose={() => setDeleteDialogOpen(false)}>
        <DialogTitle sx={{ color: "#e53935", fontWeight: "bold" }}>Delete Invoice</DialogTitle>
        <DialogContent>
          <DialogContentText>
            Permanently delete Invoice <strong>#{actionInvoice?.invoice_number}</strong>?<br />
            Stock quantities will be <strong>restored</strong>. This action cannot be undone.
          </DialogContentText>
        </DialogContent>
        <DialogActions>
