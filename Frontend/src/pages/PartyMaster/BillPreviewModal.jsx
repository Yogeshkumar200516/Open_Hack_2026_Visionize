...
function BillPreviewModal({ open, onClose, invoice, bill }) {
...
const handleSend = async () => {
  ...
  try {
    ...
    if (subscriptionType === "bill") {
      ...
      number = bill.bill_no || bill.bill_number;
      customerEmail = bill.customer_email || bill.email;
      if (!customerEmail) {
        setSnackbar({
          open: true,
      ...
      pdfResult = await sendInvoicePDF(bill, false, token);
    }
 else {
...
>
  ...
  <Box sx={{ color: "#ccc" }}>
    {[
      {
        label:
          (parseFloat(invoice.gst_percentage) || 0) > 0
      ...
      },
      {
        label:
          (parseFloat(invoice.gst_percentage) || 0) > 0
            ? `CGST (+ ${(parseFloat(invoice.gst_percentage) / 2).toFixed(2)}% on Bill)`
            : "CGST",
      ...
      },
      {
        label:
          (parseFloat(invoice.gst_percentage) || 0) > 0
            ? `SGST (+ ${(parseFloat(invoice.gst_percentage) / 2).toFixed(2)}% on Bill)`
            : "SGST",
