...
const Products = () => {
  ...
  const handleSaveProduct = async (productData) => {
    ...
    if (isNaN(gst)) gst = 0;
    if (gst < 0 || gst > 300) { showSnackbar("GST must be a number between 0 and 300.", "error"); return; }
    if (cleanedHSN.length < 4) { showSnackbar("HSN code missing or too short (min 4 characters).", "error"); return; }
  ...
  return (
    <Box sx={{
      ...
      <Box sx={{ display: "flex", justifyContent: "flex-end", mt: 2, gap: "8px", mb: "10px" }}>
        {/* Bulk Upload button — only shown in active products view */}
        {!showTracedOnly && (
              ...
              <TableHead>
                <TableRow sx={{ height: 60 }}>
                  {["ID", "Image", "Name", "Category", "Price", "Discount", "Rate", "GST", "HSN", "Stock", "Actions"].map((header) => (
                    <TableCell key={header} sx={{ color: primaryColor, fontWeight: "bold", fontSize: "0.95rem", backgroundColor: theme.palette.background.default }}>
                      {header}
                    </TableCell>
                  ))}
                </TableRow>
