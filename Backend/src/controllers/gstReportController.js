...
async function getSubscriptionType(tenant_id) {
  ...
  _subTypeCache.set(tenant_id, { type, expiresAt: now + 5 * 60 * 1000 });
  return type;
}
...
function pgDateFilter(col, dateFilter) {
  if (dateFilter === "today") return `AND ${col}::date = CURRENT_DATE`;
  if (dateFilter === "yesterday") return `AND ${col}::date = CURRENT_DATE - INTERVAL '1 day'`;
  return "";
}
...
// ─── FIX: tableNames now uses explicit, unambiguous column aliases ────────────
// The root cause of "column i.id does not exist" and "column ti.id does not exist"
// was that the UNION subquery for "both" mode did NOT expose a column named the
// same as docId in the outer query's JOIN condition. The fix is:
//   1. Use a named CTE alias `doc_id` in the UNION that always matches docId.
//   2. Use `doc_id` consistently as the join key in every outer query.
function tableNames(subType) {
  ...
  if (isBoth) {
    return {
      // UNION exposes `doc_id` — used as the join key everywhere
      docs: `(
        SELECT
          bill_id            AS doc_id,
      ...
      // UNION exposes `doc_id` as the FK to the docs subquery
      items: `(
        SELECT
          product_id,
          quantity,
      ...
      docId: "doc_id",   // ← was "id" — that column did not exist → FIXED
      isBoth: true
    };
  }
...
exports.getSummary = async (req, res) => {
  ...
  let sub = "invoice";
  try {
    sub = await getSubscriptionType(tenant_id);
    const { year, month, week, quarter, date_filter } = req.query;
    const { docs, items, docId } = tableNames(sub);

    // FIX: Use the correct docId alias ("doc_id" for both, real PK col otherwise).
    // Sub-queries now reference the correct alias consistently.
    const query = `
      SELECT
        ...
        (
          ...
          FROM ${items} i
          WHERE i.${docId} IN (
            SELECT d2.${docId} FROM ${docs} d2
            WHERE d2.tenant_id = CAST(:tenant_id AS INTEGER)
            ${pgYearFilter("d2.created_at", year)}
            ${pgMonthFilter("d2.created_at", month)}
            ${pgWeekFilter("d2.created_at", week)}
            ${pgQuarterFilter("d2.created_at", quarter)}
            ${pgDateFilter("d2.created_at", date_filter)}
          )
    ...
    const result = row || {
      ...
      total_discount: 0, total_transport: 0, total_quantity_sold: 0,
      total_products_sold: 0, avg_doc_value: 0
    };
...
exports.getMonthlyTrend = async (req, res) => {
  ...
  try {
    ...
    const query = `
      SELECT
        ${pgMonthGroup("d.created_at")}           AS month,
...
exports.getTopProducts = async (req, res) => {
  ...
  try {
    ...
    // FIX: JOIN condition now uses the correct alias from tableNames().
    // Previously used `ti.id` which did not exist in the subquery → FIXED to `ti.doc_id`
    // for "both" mode, and the real FK column for bill/invoice modes.
    const query = `
      SELECT
        p.product_id,
        p.product_name,
        ...
        COALESCE(SUM(d.discount_value),0)
          / NULLIF(COUNT(DISTINCT ti.product_id), 0)            AS total_discount_given
      FROM ${items} ti
      JOIN products p
...
exports.getDiscountsByProduct = async (req, res) => {
  ...
  try {
    ...
    const query = `
      SELECT
        p.product_name,
        COUNT(ti.${itemIdCol})                                  AS times_sold,
...
exports.getAdvanceInvoices = async (req, res) => {
  ...
  try {
    ...
    if (subscriptionType === "bill") {
      sql = `
        SELECT
          ...
          tbl.due_date,
          tbl.payment_completion_status,
          tbl.payment_settlement_date,
          tbl.customer_name,
          tbl.mobile_no AS customer_mobile
        FROM bills tbl
        WHERE tbl.tenant_id = CAST(:tenant_id AS INTEGER)
          AND tbl.payment_status = 'Advance'
      `;
    } else {
      sql = `
        SELECT
    ...
    if (year)  sql += ` ${pgYearFilter("tbl.created_at", year)}`;
    if (month) sql += ` ${pgMonthFilter("tbl.created_at", month)}`;
    ...
    sql += subscriptionType === "bill"
      ? " ORDER BY tbl.bill_date DESC"
      : " ORDER BY tbl.invoice_date DESC";
...
exports.getKpiMetrics = async (req, res) => {
  ...
  try {
    ...
    const combinedQuery = `
      WITH sales_cte AS (${salesQuery}),
           sr_cte    AS (${srQuery}),
           pr_cte    AS (${prQuery}),
           stock_cte AS (${stockQuery})
      SELECT * FROM sales_cte, sr_cte, pr_cte, stock_cte;
    `;

    const rows = await products.sequelize.query(combinedQuery, {
      replacements: { tenant_id, year, month, date_filter },
      type: Sequelize.QueryTypes.SELECT
    });
