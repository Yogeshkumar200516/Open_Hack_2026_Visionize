...
// FIX: Original destructured `[rows]` from a SELECT which returns the first row
// directly — not an array wrapper. This caused `rows` to be undefined for tenants
// whose query returned exactly one row, crashing every downstream call.
async function getSubscriptionType(tenantId) {
  const rows = await db.query(
    `SELECT subscription_type FROM company_info WHERE id = :tenant_id`,
    { replacements: { tenant_id: tenantId }, type: Sequelize.QueryTypes.SELECT }
  );
  // QueryTypes.SELECT returns an array of rows; take the first row safely.
  return rows?.[0]?.subscription_type || "invoice";
}

// FIX: tableNames now uses `doc_id` as the unified alias in the UNION subquery
// so every outer query can safely reference the same column name regardless of
// whether the tenant is in bill, invoice, or both mode.
// Additionally, `customerCol` for invoices now correctly uses `customer_id`
// (an integer FK that always exists) instead of `customer_name` (which does not
// exist on the invoices table). For the "both" UNION, a safe CAST is applied.
function tableNames(subType) {
    ...
    return {
      docs: `(
        SELECT
          ...
          transport_charge,
          payment_status::TEXT,
          payment_completion_status::TEXT,
          advance_amount,
          payment_type::TEXT,
          tenant_id,
          created_at,
        ...
        UNION ALL
        SELECT
          invoice_id                       AS doc_id,
          invoice_number                   AS bill_number,
          invoice_date                     AS doc_date,
          -- invoices store customer_id (int), not customer_name.
          -- CAST to TEXT so the UNION column types are compatible.
      ...
      docId:       "doc_id",
      customerCol: "customer_name",   // safe — the UNION exposes this alias
      isBoth: true
    };
  }
  ...
  return {
    ...
    // FIX: invoices table has customer_id (FK), NOT customer_name.
    // Using customer_name on the invoices table caused a column-not-found
    // error for tenants in invoice mode → switched to customer_id.
    customerCol: isBill ? "customer_name" : "customer_id",
    isBoth: false
  };
}
...
exports.getSummary = async (req, res) => {
  ...
  try {
    ...
    const [rev] = await db.query(
      `SELECT
         COALESCE(SUM(CASE WHEN EXTRACT(MONTH FROM created_at)=EXTRACT(MONTH FROM NOW())
           AND EXTRACT(YEAR FROM created_at)=EXTRACT(YEAR FROM NOW())
           THEN total_amount END),0) AS curr_rev,
         COALESCE(SUM(CASE WHEN EXTRACT(MONTH FROM created_at)=EXTRACT(MONTH FROM (NOW() - INTERVAL '1 month'))
         ...
         COALESCE(SUM(CASE WHEN payment_status='Advance' AND payment_completion_status='Pending'
           THEN total_amount - advance_amount END),0) AS pending_dues,
         COUNT(DISTINCT CASE WHEN EXTRACT(MONTH FROM created_at)=EXTRACT(MONTH FROM NOW())
           AND EXTRACT(YEAR FROM created_at)=EXTRACT(YEAR FROM NOW())
           THEN ${docId} END) AS curr_docs,
    ...
    const [retRow] = await db.query(
      `SELECT
         COALESCE(SUM(CASE WHEN EXTRACT(MONTH FROM created_at)=EXTRACT(MONTH FROM NOW())
           AND EXTRACT(YEAR FROM created_at)=EXTRACT(YEAR FROM NOW())
           THEN total_amount END),0) AS curr,
         COALESCE(SUM(CASE WHEN EXTRACT(MONTH FROM created_at)=EXTRACT(MONTH FROM (NOW() - INTERVAL '1 month'))
           AND EXTRACT(YEAR FROM created_at)=EXTRACT(YEAR FROM (NOW() - INTERVAL '1 month'))
           THEN total_amount END),0) AS prev
...
exports.getSalesForecast = async (req, res) => {
  ...
  try {
    ...
    if (rows.length < 2) {
      return res.json({
        forecast: null,
        message: "Need at least 2 months of data. Found " + rows.length + ".",
        historical: rows
      });
    }
    ...
    res.json({
      ...
      analytics: {
        ...
        growth_rates: rows.slice(1).map((r, i) => ({
          month:      r.month,
          growth_pct: growthRates[i] ? fmt2(growthRates[i]) : null
        })),
      },
      historical: rows,
    });
...
exports.getReturnSpike = async (req, res) => {
  ...
  try {
    ...
    const salesVsReturn = await db.query(
      `SELECT
       ...
       LEFT JOIN sales_returns sr
         ON sr.original_invoice_id = d.${docId}
    ...
    res.json({
      ...
      summary_message:  allSpikes.length > 0
        ? "⚠️ " + allSpikes.length + " return spike(s) detected. Immediate review required."
        : "✅ Return rates are within normal bounds.",
      spikes:           allSpikes,
  ...
  try {
    ...
    const velocity = await db.query(
      `SELECT
       ...
       LEFT JOIN ${docs} d ON d.${docId} = i.${docId}
         AND d.created_at >= (NOW() - INTERVAL '30 days')
       LEFT JOIN product_categories c ON c.category_id = p.category_id
       WHERE p.tenant_id = :tenantId
       GROUP BY p.product_id, p.product_name, p.category_id, c.category_name,
                p.stock_quantity, p.sellable_stock, p.price, p.damaged_stock, p.scrap_stock`,
      { replacements: { tenantId }, type: Sequelize.QueryTypes.SELECT }
    );
    ...
    const incMap = {};
    incoming.forEach((m) => { incMap[m.product_id] = Number(m.incoming_qty); });
    ...
    const vMap = {};
    vendors.forEach((v) => { if (!vMap[v.product_id]) vMap[v.product_id] = v; });
    ...
    const retMap = {};
    retRates.forEach((r) => { retMap[r.product_id] = Number(r.returned_qty); });
...
exports.getBusinessInsights = async (req, res) => {
  ...
  try {
    ...
    // FIX: COUNT(DISTINCT customerCol) — for invoices this was previously
    // `customer_name` which does not exist on the invoices table.
    // Now correctly uses `customer_id` for invoice mode (set in tableNames).
    const [curr] = await db.query(
      `SELECT
         ...
         COALESCE(SUM(CASE WHEN payment_status='Advance'
    ...
    const [pendingVer] = await db.query(
      `SELECT COUNT(*) AS cnt
       FROM return_stock_verification
       WHERE tenant_id = :tenantId AND verification_status = 'pending'`,
      { replacements: { tenantId }, type: Sequelize.QueryTypes.SELECT }
    );
    ...
    const payments = await db.query(
      `SELECT
         COALESCE(payment_type::text, 'Not Specified') AS payment_type,
         COUNT(*)                                       AS count,
    ...
    const dailySales = await db.query(
      `SELECT
         EXTRACT(DAY FROM created_at)          AS day_num,
       ...
       WHERE d.tenant_id = :tenantId
         AND EXTRACT(MONTH FROM d.created_at) = EXTRACT(MONTH FROM NOW())
         AND EXTRACT(YEAR  FROM d.created_at) = EXTRACT(YEAR  FROM NOW())
       GROUP BY day_num, trim(to_char(d.created_at, 'Day'))
       ORDER BY day_num`,
      { replacements: { tenantId }, type: Sequelize.QueryTypes.SELECT }
    );
    ...
    const avgOrderGrowth   = Number(prev?.avg_order) > 0
      ? ((Number(curr?.avg_order) - Number(prev?.avg_order)) / Number(prev?.avg_order)) * 100 : null;
    const customerGrowth   = Number(prev?.unique_customers) > 0
      ? ((Number(curr?.unique_customers) - Number(prev?.unique_customers)) / Number(prev?.unique_customers)) * 100 : null;
    ...
    const kpis = {
      ...
      gross_revenue:        fmt2(revenue),
      net_revenue:          fmt2(netRevenue),
      total_gst:            fmt2(curr?.gst),
      gst_collected:        fmt2(curr?.gst),
      cgst:                 fmt2(curr?.cgst),
      sgst:                 fmt2(curr?.sgst),
...
exports.getCustomerSegmentation = async (req, res) => {
  ...
  try {
    ...
    customersList.sort((a, b) => b.monetary - a.monetary);
    ...
    const segments = Object.values(segMap).map((s) => ({
      ...
      share_pct:     fmt2((s.count / customersList.length) * 100),
    })).sort((a, b) => b.count - a.count);

    const totalCLV = customersList.reduce((a, b) => a + Number(b.clv_annual_estimate), 0);
    const avgCLV   = customersList.length > 0 ? totalCLV / customersList.length : 0;

    res.json({
      summary: {
        total_customers:        customersList.length,
        total_customer_revenue: fmt2(customersList.reduce((a, b) => a + Number(b.monetary), 0)),
        avg_customer_value:     fmt2(customersList.reduce((a, b) => a + Number(b.monetary), 0) / customersList.length),
        ...
        avg_frequency:          fmt2(customersList.reduce((a, b) => a + b.frequency, 0) / customersList.length),
        avg_recency_days:       fmt2(customersList.reduce((a, b) => a + b.recency_days, 0) / customersList.length),
      },
      segments,
      customers: customersList.slice(0, 100),
    });
  } catch (err) {
  ...
  try {
    ...
    const velocity90 = await db.query(
      `SELECT
       ...
       GROUP BY p.product_id, p.product_name, p.price, p.stock_quantity,
                p.sellable_stock, p.damaged_stock, p.scrap_stock, p.hsn_code, c.category_name`,
      { replacements: { tenantId }, type: Sequelize.QueryTypes.SELECT }
    );
    ...
    const analytics = velocity90.map((p) => {
      ...
      const daysSinceLastSale = lastSoldDate
        ? Math.floor((Date.now() - new Date(lastSoldDate).getTime()) / 86400000)
        : null;
    ...
    res.json({
      summary: {
        ...
        avg_return_rate:      analytics.length > 0
          ? fmt2(analytics.reduce((s, a) => s + Number(a.return_rate_pct), 0) / analytics.length)
          : 0,
      },
...
exports.getGstAnalytics = async (req, res) => {
  ...
  try {
    ...
    const topGSTProducts = await db.query(
      `SELECT
         p.product_name, p.hsn_code, i.gst_percentage,
    ...
    if (sub !== "bill") {
      const docTypeRows = await db.query(
        `SELECT
           COALESCE(document_type::text, 'Tax Invoice') AS document_type,
           COUNT(*)                                      AS count,
  ...
  try {
    ...
    const inflows = await db.query(
      `SELECT
         ...
         COALESCE(SUM(CASE WHEN payment_status='Advance' AND payment_completion_status='Pending'
           THEN total_amount - advance_amount END),0) AS receivable,
         COALESCE(SUM(CASE WHEN payment_status='Advance' AND payment_completion_status='Completed'
           THEN total_amount - advance_amount END),0) AS collected_dues,
    ...
    const agingRows = await db.query(
      `SELECT
       ...
       FROM ${docs} AS d WHERE d.tenant_id = :tenantId
         AND payment_status = 'Advance' AND payment_completion_status = 'Pending'`,
      { replacements: { tenantId }, type: Sequelize.QueryTypes.SELECT }
    );
    ...
    const outMap = {};
    outflows.forEach((r) => (outMap[r.month] = Number(r.return_outflow)));
    ...
    res.json({
      summary: {
        ...
        avg_monthly_inflow: cashFlow.length > 0
          ? fmt2(cashFlow.reduce((a, b) => a + Number(b.cash_inflow), 0) / cashFlow.length)
          : 0,
      },
...
exports.getDiscountAnalysis = async (req, res) => {
  ...
  try {
      ...
      const cols = await db.query(
        `SELECT column_name FROM information_schema.columns
         WHERE table_schema = current_schema()
           AND table_name = :docs
           AND column_name IN ('discount_value','discount_amount','discount','discount_type','total_with_gst')`,
      ...
      const itemCols = await db.query(
        `SELECT column_name FROM information_schema.columns
         WHERE table_schema = current_schema()
           ...
           AND column_name IN ('total_with_gst','total_amount','gst_percentage','quantity')`,
        { replacements: { items }, type: Sequelize.QueryTypes.SELECT }
      );
      itemColNames = itemCols.map((c) => c.column_name);
    }

    const discCol      = colNames.includes("discount_value")  ? "discount_value"
                       : colNames.includes("discount_amount") ? "discount_amount"
                       : colNames.includes("discount")        ? "discount"
                       : null;
    ...
    if (!discCol) {
      return res.json({
        summary: {
          discount_effectiveness:     "no_data",
          avg_order_lift_pct:         0,
          discounted_avg_order:       0,
          non_discounted_avg_order:   0,
          total_discount_given_6m:    "0",
          total_revenue_6m:           "0",
          overall_discount_rate_pct:  0,
          discounted_invoice_share:   0,
    ...
    res.json({
      summary: {
        ...
        discounted_invoice_share:   discounted && nonDiscounted
          ? fmt2((Number(discounted.invoice_count) / (Number(discounted.invoice_count) + Number(nonDiscounted.invoice_count))) * 100)
          : 0,
        recommendation: Number(discountLift) > 5
          ? "Discounts are generating higher order values. Strategy is working."
          : Number(discountLift) > 0
            ? "Mild positive lift from discounts. Consider targeted offers."
      ...
      product_discounts: productDiscount.map((r) => ({
        ...
        discount_to_revenue_pct: Number(r.total_revenue) > 0
          ? fmt2((Number(r.total_discount_given) / Number(r.total_revenue)) * 100)
          : 0,
      })),
  ...
  try {
    ...
    const monthlyAgg = await db.query(
      `SELECT
         ...
         COALESCE(SUM(total_amount),0)             AS total_revenue
       FROM ${docs} AS d WHERE d.tenant_id = :tenantId
       GROUP BY month_num, trim(to_char(created_at, 'Month'))
       ORDER BY month_num`,
      { replacements: { tenantId }, type: Sequelize.QueryTypes.SELECT }
    );
    ...
    const dowAgg = await db.query(
      `SELECT
         EXTRACT(DOW FROM created_at) + 1         AS dow_num,
         trim(to_char(created_at, 'Day'))         AS dow_name,
         COUNT(*)                                 AS total_docs,
         COALESCE(AVG(total_amount),0)            AS avg_revenue,
         COALESCE(SUM(total_amount),0)            AS total_revenue
       FROM ${docs} AS d WHERE d.tenant_id = :tenantId
       GROUP BY dow_num, trim(to_char(created_at, 'Day'))
       ORDER BY dow_num`,
      { replacements: { tenantId }, type: Sequelize.QueryTypes.SELECT }
    );
    ...
    const hourAgg = await db.query(
      `SELECT
         EXTRACT(HOUR FROM d.created_at)          AS hour_num,
         COUNT(*)                                 AS total_docs,
         COALESCE(AVG(total_amount),0)            AS avg_revenue
       FROM ${docs} AS d WHERE d.tenant_id = :tenantId
    ...
    const avgMonthly = monthlyAgg.length > 0
      ? monthlyAgg.reduce((a, r) => a + Number(r.avg_revenue), 0) / monthlyAgg.length
      : 0;
    ...
    res.json({
      summary: {
        ...
        recommendation: peakMonths.length > 0
          ? "Peak season: " + peakMonths.join(", ") + ". Stock up and run promotions. " +
            (slowMonths.length > 0 ? "Slow months: " + slowMonths.join(", ") + " — consider clearance sales." : "")
